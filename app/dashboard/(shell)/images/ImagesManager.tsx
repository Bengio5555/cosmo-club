  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[200px_minmax(0,1fr)]">
      ...
      <aside className="space-y-2">
        ...
        {pages.length === 0 ? (
          <p className="text-xs text-slate-500 dark:text-slate-500">Aucune page configurée.</p>
        ) : (
                ...
                <button
                  ...
                  className={`text-left rounded-md px-3 py-2 text-xs font-medium transition-colors ${
                    active
                      ...
                      : "border border-slate-300 bg-white dark:border-slate-800 dark:bg-slate-900 text-slate-700 hover:border-slate-400 hover:text-slate-900 dark:text-slate-300 dark:hover:border-slate-700 dark:hover:text-white"
                  }`}
                >
                  {page}
                </button>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between gap-3">
          ...
          <button
            ...
            disabled={pending}
            className="inline-flex items-center gap-1 rounded-md border border-slate-300 bg-white dark:border-slate-800 dark:bg-slate-900 px-2.5 py-1 text-[11px] text-slate-700 hover:border-slate-400 hover:text-slate-900 dark:text-slate-300 dark:hover:border-slate-700 dark:hover:text-white disabled:opacity-50"
          >
        ...
        <div className="grid gap-4 lg:h-[calc(100vh-260px)] lg:grid-cols-[320px_minmax(0,1fr)]">
          ...
          <section className="flex min-h-0 flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-950/60 dark:shadow-none">
            <div className="border-b border-slate-100 dark:border-slate-900 p-3">
              <input
                ref={fileInputRef}
                type="file"
                accept="image/*"
              ...
              <label
                ...
                className={`block cursor-pointer rounded-md border-2 border-dashed border-[color:var(--color-grenat)]/60 bg-slate-100 dark:bg-slate-900 p-3 text-center transition-colors ${
                  pending ? "opacity-50" : "hover:bg-slate-200 dark:hover:bg-slate-800"
                }`}
              >
            ...
            <div className="min-h-0 flex-1 overflow-y-auto p-2">
              {imageEntries.length === 0 ? (
                <p className="p-4 text-center text-xs text-slate-500 dark:text-slate-500">
                  Aucune image
                </p>
              ) : (
                      ...
                      <li key={key}>
                        <button
                          ...
                          className={`flex w-full gap-3 rounded-md p-2 text-left transition-colors ${
                            active
                              ? "border-2 border-[color:var(--color-grenat)] bg-slate-100 dark:bg-slate-900"
                              : "border-2 border-transparent bg-slate-100 dark:bg-slate-900/50 hover:bg-slate-100 dark:hover:bg-slate-900"
                          }`}
                        >
                          <div className="h-14 w-14 shrink-0 overflow-hidden rounded bg-slate-200 dark:bg-slate-800">
                            {image.path ? (
          ...
          {/* Right: detail panel */}
          <section className="overflow-y-auto rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-950/60 dark:shadow-none">
  ...
  return (
    <div className="space-y-5 p-5">
      ...
      <div className="space-y-2">
        ...
        <div
          className="w-full overflow-hidden rounded-lg bg-slate-100 dark:bg-slate-900"
          style={{ aspectRatio }}
        >
          {image.path && (
            // eslint-disable-next-line @next/next/no-img-element
      ...
      <div className="space-y-2">
        ...
        {editingTitle ? (
          ...
          <button
            ...
            className="w-full rounded-md border border-slate-300 bg-white dark:border-slate-800 dark:bg-slate-900 px-3 py-2 text-left text-sm text-slate-900 dark:text-white transition-colors hover:border-slate-300 dark:hover:border-slate-700"
          >
            {image.title} <span className="text-slate-500 dark:text-slate-500">✏️</span>
          </button>
        )}
      </div>
      ...
      <div className="space-y-2 rounded-lg border border-[color:var(--color-grenat)]/40 bg-[color:var(--color-grenat)]/10 p-3">
        ...
        <select
          ...
          disabled={pending}
          className="w-full rounded-md border border-[color:var(--color-grenat)]/40 bg-slate-100 dark:bg-slate-900 px-3 py-2 text-sm font-semibold text-[color:var(--color-grenat-glow)] focus:border-[color:var(--color-grenat)] focus:outline-none disabled:opacity-50"
        >
            ...
            return (
              <button
                ...
                className={`rounded-md px-3 py-2 text-xs font-semibold transition-colors disabled:opacity-50 ${
                  active
                    ? "bg-[color:var(--color-grenat)] text-[color:var(--color-bone)]"
                    : "border border-slate-300 bg-white dark:border-slate-800 dark:bg-slate-900 text-slate-600 dark:text-slate-300 hover:border-slate-300 dark:hover:border-slate-700"
                }`}
              >
