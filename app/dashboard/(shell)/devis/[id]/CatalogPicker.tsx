...
export function CatalogPicker({
  ...
  // Section the picked items land in. Driven entirely by where the
  // picker was opened from: when launched inside a quote section,
  // `defaultSection` is that section's name and the items go there;
  // when launched from the global "Catalogue" button it's undefined,
  // and each item keeps its own catalog section. There's no longer a
  // visible input for this — it was confusing in the global picker.
  const targetSection = defaultSection ?? "";
          ...
          <div className="border-b border-slate-200 dark:border-slate-800 px-4 py-3">
            ...
            {defaultSection && (
              <p className="mt-2 text-[11px] text-slate-500 dark:text-slate-500">
                Les prestations seront ajoutées à la section{" "}
