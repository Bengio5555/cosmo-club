...
export function GalleryPicker({ tiles }: { tiles: DecoratedTile[] }) {
  ...
  if (tiles.length === 0) {
    return (
      <p className="rounded-md border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950/60 p-6 text-sm text-slate-600 dark:text-slate-400">
        ...
        {ordered.map((tile) => (
          <li
            ...
            className={`group relative overflow-hidden rounded-xl border transition-colors ${
              tile.selected
                ? "border-[color:var(--color-grenat)] bg-slate-100 dark:bg-slate-900/60"
                ...
                <button
                  ...
                  className={`inline-flex items-center gap-1 rounded-md border px-2.5 py-1 text-[11px] font-semibold transition-colors disabled:opacity-50 ${
                    tile.selected
                      ...
                      : "border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-900 text-slate-700 hover:border-slate-400 hover:text-slate-900 dark:text-slate-300 dark:hover:border-slate-700 dark:hover:text-white"
                  }`}
                >
                  <Star
