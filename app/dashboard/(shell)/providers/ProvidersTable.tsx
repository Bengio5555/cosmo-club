      ...
      <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <p className="text-xs text-slate-500 dark:text-slate-500">
          ...
          {(cat !== "all" || normalized) && providers.length > 0 && (
            <span className="ml-1 text-slate-400 dark:text-slate-600"> · sur {providers.length} au total</span>
          )}
        </p>
        <div className="flex flex-wrap items-center gap-2">
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-md border border-slate-300 bg-white dark:border-slate-800 dark:bg-slate-900 py-2 pl-8 pr-2.5 text-xs text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-600 focus:border-[color:var(--color-grenat)] focus:outline-none md:w-72"
            />
          </div>
          <button
            type="button"
      ...
      <div className="space-y-4">
        {providers.length === 0 ? (
          ...
          grouped.map(([catName, list]) => (
            <div key={catName} className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-950/60 dark:shadow-none">
    ...
    <li className={`flex flex-col gap-2 px-4 py-3 ${p.archived ? "opacity-50" : ""} md:flex-row md:items-center md:gap-3`}>
      <div className="min-w-0 flex-1">
        ...
        {(p.pricing_info || p.notes) && (
          <p className="mt-1 max-w-2xl whitespace-pre-line text-[11px] text-slate-500 dark:text-slate-500 line-clamp-3">
            {p.pricing_info ?? p.notes}
          </p>
        )}
      </div>
      <div className="flex flex-wrap items-center gap-1.5 text-xs">
        {p.email && (
          <a
            href={`mailto:${p.email}`}
            className="inline-flex items-center gap-1 rounded-md border border-slate-300 bg-white dark:border-slate-800 dark:bg-slate-900 px-2 py-1 text-slate-700 hover:border-slate-400 hover:text-slate-900 dark:text-slate-300 dark:hover:border-slate-700 dark:hover:text-white"
          >
        ...
        <button
          ...
          onClick={onEdit}
          disabled={pending}
          className="rounded p-1.5 text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white"
          aria-label="Modifier"
        >
        ...
        <button
          ...
          onClick={onDelete}
          disabled={pending}
          className="rounded p-1.5 text-slate-500 dark:text-slate-500 hover:bg-red-100 dark:hover:bg-red-500/10 hover:text-red-300"
    ...
    >
      ...
      <span
        className={`rounded-full px-1.5 text-[10px] font-mono ${
          active ? "bg-[color:var(--color-grenat)]/30" : "bg-slate-200 dark:bg-slate-800"
        }`}
      >
        {count}
      </span>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 p-5 shadow-2xl">
        <div className="mb-3 flex items-start justify-between">
          ...
          <button type="button" onClick={onClose} className="rounded-md p-1 text-slate-500 dark:text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-900 hover:text-slate-900 dark:hover:text-white" aria-label="Fermer">
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <form action={onSubmit} className="space-y-3">
          ...
          <div className="mt-4 flex items-center justify-end gap-2 border-t border-slate-100 dark:border-slate-900 pt-4">
            <button type="button" onClick={onClose} disabled={pending} className="rounded-md border border-slate-300 bg-white dark:border-slate-800 dark:bg-slate-900 px-3 py-2 text-xs text-slate-600 dark:text-slate-300 hover:border-slate-300 dark:hover:border-slate-700">
              Annuler
            </button>
            <button type="submit" disabled={pending} className="inline-flex items-center gap-1.5 rounded-md bg-slate-900 px-3 py-2 text-sm font-medium text-white shadow-sm transition-colors hover:bg-slate-800 dark:bg-slate-100 dark:text-slate-900 dark:hover:bg-white disabled:opacity-60">
