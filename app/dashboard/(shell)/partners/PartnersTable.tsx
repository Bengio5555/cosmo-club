      ...
      <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <p className="text-xs text-slate-500 dark:text-neutral-500">
          ...
          {(cat !== "all" || normalized) && partners.length > 0 && (
            <span className="ml-1 text-slate-400 dark:text-neutral-600"> · sur {partners.length} au total</span>
          )}
        </p>
  ...
  return (
    <li className={`flex flex-col gap-2 px-4 py-3 ${p.archived ? "opacity-50" : ""} md:flex-row md:items-center md:gap-3`}>
      ...
      <div className="flex flex-wrap items-center gap-1.5 text-xs">
        ...
        <button type="button" onClick={onEdit} disabled={pending} className="rounded p-1.5 text-slate-500 dark:text-neutral-400 hover:bg-slate-200 dark:hover:bg-neutral-800 hover:text-slate-900 dark:hover:text-white" aria-label="Modifier">
          <Pencil className="h-3.5 w-3.5" />
        </button>
        <button type="button" onClick={onArchive} disabled={pending} className="rounded p-1.5 text-slate-500 dark:text-neutral-500 hover:bg-slate-200 dark:hover:bg-neutral-800 hover:text-neutral-200" aria-label={p.archived ? "Désarchiver" : "Archiver"} title={p.archived ? "Désarchiver" : "Archiver"}>
          {p.archived ? <ArchiveRestore className="h-3.5 w-3.5" /> : <Archive className="h-3.5 w-3.5" />}
        </button>
        <button type="button" onClick={onDelete} disabled={pending} className="rounded p-1.5 text-slate-500 dark:text-neutral-500 hover:bg-red-500/10 hover:text-red-300" aria-label="Supprimer" title="Supprimer définitivement">
