...
export default async function ClientsListPage({
  ...
  return (
    <div className="px-6 py-6 md:px-10 md:py-8">
      ...
      <form
        method="GET"
        className="mb-2 flex items-center gap-2 rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-950/60 dark:shadow-none p-2.5"
      >
        <Search className="ml-1 h-3.5 w-3.5 text-slate-400 dark:text-slate-500" />
        <input
          type="search"
          name="q"
          defaultValue={query}
          placeholder="Rechercher par nom, société, email…"
          className="flex-1 bg-transparent text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none dark:text-white dark:placeholder:text-slate-600"
        />
        {showArchived && <input type="hidden" name="archived" value="1" />}
        {query && (
          <Link
            href={showArchived ? "/dashboard/clients?archived=1" : "/dashboard/clients"}
            className="rounded-md border border-transparent px-2 py-1 text-[11px] text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white"
          >
            Effacer
          </Link>
        )}
        <button
          type="submit"
          className="rounded-md border border-slate-300 bg-white px-3 py-1 text-xs font-semibold text-slate-700 shadow-sm hover:border-slate-400 hover:text-slate-900 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-100 dark:hover:border-slate-700 dark:hover:text-white dark:shadow-none"
        >
          Filtrer
        </button>
      </form>

      <div className="mb-4 flex items-center gap-3 text-[11px] text-slate-500 dark:text-slate-500">
        {showArchived ? (
          <Link
            href="/dashboard/clients"
            className="inline-flex items-center gap-1 rounded-full border border-slate-300 bg-white px-2.5 py-1 text-slate-700 shadow-sm hover:border-slate-400 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300 dark:hover:border-slate-600 dark:shadow-none"
          >
          ...
          (archivedCount ?? 0) > 0 && (
            <Link
              ...
              className="inline-flex items-center gap-1 rounded-full border border-slate-300 bg-white px-2.5 py-1 text-slate-500 shadow-sm hover:border-slate-400 hover:text-slate-700 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-400 dark:hover:border-slate-600 dark:hover:text-slate-200 dark:shadow-none"
            >
      ...
      <div className="table-as-cards overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-950/60 dark:shadow-none">
        {sortedClients.length > 0 ? (
                      ...
                      >
                        ...
                        <div className="min-w-0">
                          <p className="inline-flex items-center gap-2 font-medium text-slate-900 transition-colors hover:text-[color:var(--color-grenat)] dark:text-white dark:hover:text-[color:var(--color-grenat-glow)]">
                            ...
                            {isB2B && (
                              <span className="rounded-full border border-slate-300 bg-slate-100 px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-wide text-slate-600 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-400">
                                B2B
                              </span>
                            )}
                          </p>
                          ...
                          <p className="truncate text-[11px] text-slate-500 dark:text-slate-500 md:hidden">
                            {c.email ?? c.phone ?? "—"}
                          </p>
                        </div>
                      </Link>
          ...
          <div className="p-8 text-center text-sm text-slate-500 dark:text-slate-500">
            Aucun client ne correspond à <span className="font-medium text-slate-700 dark:text-slate-300">“{query}”</span>.
          </div>
        ) : (
          <div className="flex flex-col items-center gap-3 p-10 text-center text-sm text-slate-500 dark:text-slate-500">
            <Users className="h-6 w-6 text-slate-400 dark:text-slate-700" />
