            ...
            <ul className="divide-y divide-slate-100 dark:divide-slate-900">
              {archived.map((c) => (
                <li key={c.id}>
                  <Link
                    href={`/dashboard/cocktails/${c.id}`}
                    className="flex items-center gap-3 px-4 py-2 text-sm text-slate-500 dark:text-slate-500 opacity-70 transition-colors hover:bg-slate-100 dark:hover:bg-slate-900 hover:text-slate-300"
                  >
                    {c.name}
                  </Link>
                </li>
