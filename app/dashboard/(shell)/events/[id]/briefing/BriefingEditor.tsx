  ...
  return (
    <div className="space-y-5">
      ...
      {cocktails.length > 0 && (
        ...
        >
          <label className="mb-3 flex cursor-pointer items-center justify-between gap-3 rounded-md border border-slate-200 bg-slate-50 px-3 py-2.5 dark:border-slate-800 dark:bg-slate-900/40">
            <span className="text-sm text-slate-700 dark:text-slate-200">
              Afficher le menu des boissons dans le briefing staff
            </span>
            <Toggle
              checked={data.show_drinks_menu}
              onChange={(v) => patch({ show_drinks_menu: v })}
            />
          </label>

          {!data.show_drinks_menu ? (
            <p className="text-xs text-slate-500 dark:text-slate-500">
              Le menu des boissons est <strong>masqué</strong> sur le briefing
              partagé au staff. Active le toggle pour l&apos;afficher.
            </p>
          ) : (
